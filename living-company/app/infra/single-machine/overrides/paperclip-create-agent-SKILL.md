...
- **adapter type — ALWAYS `codex_local`. NEVER `claude_local`, `opencode_local`, `cursor`, or any other adapter.** This environment only has the Azure brain wired for codex_local; `claude_local` needs an Anthropic key we do not have, and `opencode_local` has no model configured — both fail every run. Every hire MUST be `codex_local` with the brain config below.
...
- **heartbeat by tier (keeps the company autonomous without overloading the model API):** leadership hires (those getting `canCreateAgents: true`) get a live timer — `runtimeConfig.heartbeat.enabled=true`, `intervalSec: 120`, `maxConcurrentRuns: 20`. Individual-contributor hires get `runtimeConfig.heartbeat = {enabled: false, wakeOnDemand: true}` — they run when you (their manager) assign work and wake them, not on a timer. This mirrors a real org: leads drive, ICs execute on assignment.
- **give every hire the same brain as you**: copy your own `adapterConfig` verbatim — the same `model` (`gpt-5.3-codex`) and the full `extraArgs`, which MUST include `-c model="gpt-5.3-codex"` (pins the model in codex config so fresh sessions don't fall back to an undeployed variant), the Azure `model_provider` overrides, and the `tex-memory` MCP wiring. Never drop the `extraArgs`; a hire without them has no working model and will fail every run.
...
- capabilities
- **hiring authority** — for leadership-tier roles (C-suite, VP, head, director, lead, or any role that will own its own reports), set `"permissions": {"canCreateAgents": true}` in the hire request so this manager can staff and grow their own team without routing every hire through the CEO. Leave it off (the default) for individual-contributor roles. Note: only the CEO can change permissions after creation, so set this at hire time for managers.
...
curl -sS -X POST "$PAPERCLIP_API_URL/api/companies/$PAPERCLIP_COMPANY_ID/agent-hires" \
  ...
  -d '{
    ...
    "adapterConfig": {"model": "gpt-5.3-codex", "dangerouslyBypassApprovalsAndSandbox": true, "extraArgs": ["...copy YOUR OWN extraArgs verbatim: -c model=\"gpt-5.3-codex\" + the Azure model_provider overrides + the tex-memory MCP wiring..."]},
